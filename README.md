...
#### Stage 1: Binary Dasymetric (BD) interpolation of habitable zones

In the first stage, "habitated zones" are interpolated from a combination of 2010 U.S. Census Bureau [TIGER/Line shapefiles](https://www.census.gov/geographies/mapping-files/time-series/geo/tiger-line-file.html) and the 2001 MRLC [NLCD percent developed imperviousness](https://www.mrlc.gov/data/nlcd-2001-percent-developed-imperviousness-conus) and [NLCD land cover](https://www.mrlc.gov/data/nlcd-2001-land-cover-conus) rasters, following the methodology from IPUMS.

Habitated zones are calculated as follows:
...
The second stage generates weights via the TDW method described in Schroeder, 2007, incorporating the habitated zones from Stage 1 in a manner similar to both IPUMS and Ruther et al., 2015. The original TDW formula is given in Schroeder, 2007 as:
...
* Subscripts t and tau refer to the "target" universe, i.e. that which the original data is being crosswalked *to* (the 2010 TIGER/Line shapefiles); and
* The combination of subscripts s+t or s+tau refers to the intersection of geographies between the source and target regionalizations.

We slightly depart from Schroeder by using the areas of the habitated zones produced in Stage 1 as follows:

![](static/tdw2.svg) (Eq. 2)

where A\* now refers to the BD-interpolated areas rather than the total areas of specific geographies. This can be thought of as Binary Dasmetric Target-Density Weighting (BD-TDW) In this way, we are able to generate weights that are more representative of the geospatial distribution of population that would be possible using TIGER/Line alone.
...
In C-BD-TDW, we first start with the strictest definition of habitated zones - presented above - in the calculation of A\*. In geographies where A\* is zero due to imperviousness restrictions, we again redefine the TDW formula as

![](static/tdw3.svg) (Eq. 3)

where A<sup>r</sup> now refers to a definition of habitated zones involving only the 300-meter road buffers, without any restriction on land cover.

In cases where A<sup>r</sup> is zero due to no roads, we finally fall back to the original TDW formula (Eq. 1).

In short, in stage 3 we combine the methodologies of IPUMS and Ruther et al., 2015. in that we use only the raw TDW formula with the BD-interpolated habitated zones, as per Ruther et al., 2015, and that we are use the habitated zones definition and cascading of progressively less-strict habitated zone definitions as per IPUMS.
